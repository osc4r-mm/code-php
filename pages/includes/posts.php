<section class="posts">
    ...
    if (!empty($search_query)) {
        echo "<h1>Resultats de la cerca: " . htmlspecialchars($search_query) . "</h1>";
        
        // Consulta para contar total de resultados
        $count_sql = "SELECT COUNT(*) AS total FROM entrades WHERE LOWER(titol) LIKE LOWER(?)";
        ...
        $sql = "
            ...
            WHERE LOWER(e.titol) LIKE LOWER(?)
            ORDER BY e.data DESC
            LIMIT ? OFFSET ?
        ";
        $stmt = $db->prepare($sql);
        $stmt->bind_param("sii", $search_term, $posts_per_page, $offset);
        $stmt->execute();
    ...
    // Parametres per buscar
    $category_param = $current_category ? "&categoria=" . $current_category : "";
    $search_param = $search_query ? "&search=" . urlencode($search_query) : "";
    ...
    if ($current_page > 1) {
        echo "<a href='index.php?pagination=".($current_page - 1).$category_param.$search_param."' class='pagination-arrow'>&lt;--</a>";
    }

    // Mostra el número 1 i punts suspensius si és necessari
    if ($current_page > 2) {
        echo "<a href='index.php?pagination=1&categoria=".$category_param.$search_param."'>1</a>";
    }
    if ($current_page > 3) {
    ...
    for ($i = $start; $i <= $end; $i++) {
        if ($i == $current_page) {
            ...
            echo "<a href='index.php?pagination=$i".$category_param.$search_param."'>$i</a>";
        }
    }
    ...
    if ($current_page < $total_pages - 1) {
        echo "<a href='index.php?pagination=$total_pages".$category_param.$search_param."'>$total_pages</a>";
    }
